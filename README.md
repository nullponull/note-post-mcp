...
### マガジン一括登録（add-to-magazine.cjs）

記事をタイトルのパターンマッチでマガジンに自動振り分け:

```bash
# 2026年1月の記事をマガジンに振り分け
node add-to-magazine.cjs 2026 1

# 特定マガジンのみ
node add-to-magazine.cjs 2026 1 --magazine "AI論文読み放題"

# 全記事を強制的に指定マガジンに登録
node add-to-magazine.cjs 2026 1 --magazine "副業×AI" --force-all

# 全期間対象
node add-to-magazine.cjs 2026 1 --all

# ドライラン
node add-to-magazine.cjs 2026 1 --dry-run
```

**マガジン判定ルール:**
| マガジン | パターン |
|---------|---------|
| AI論文読み放題 | 論文, 翻訳, Paper, Research, GPT-SoVITS, 音声合成 等 |
| 副業×AI | 副業, ニュース×AI, 株, 経済, 投資, DMM, 楽天 |

### メンバーシップ特典一括追加（add-membership-benefit.cjs）

記事をメンバーシップ特典として一括登録:

```bash
node add-membership-benefit.cjs 2026 1
node add-membership-benefit.cjs 2026 1 --dry-run
node add-membership-benefit.cjs 2026 1 --limit 10
```
...
├── batch-publish.cjs              # バッチ投稿スクリプト（汎用）
├── batch-publish-october.cjs      # 10月版バッチ（成功パターン）
├── batch-publish-november.cjs     # 11月版バッチ
├── batch-publish-december.cjs     # 12月版バッチ
├── add-to-magazine.cjs            # マガジン一括登録
├── add-membership-benefit.cjs     # メンバーシップ特典一括追加
├── package.json
├── tsconfig.json
└── README.md
```
